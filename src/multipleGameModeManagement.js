...
function selectPlayer() {
    ...
    if (checkingPlayerIsSelected.checked == true) {
        ...
        if (selectedGameModeInMultipleMode["Player"] == 0 && selectedGameModeInMultipleMode["Banker"] == 0 &&
            selectedGameModeInMultipleMode["Differential"] == 0 && selectedGameModeInMultipleMode["Target"] == 0 &&
...
// Checking Differential Is Selected Or Not 
function selectDifferential() {

    //if (selectedGameModeInMultipleMode['All'] == 1) return;

    document.getElementById('displayNoOptionSelected').style.display = "none";

    if (IschangePreferencesOn == 1) var checkingDifferentialIsSelected = document.getElementById('MaltipleGameModeOnDifferentialINCP');
    else var checkingDifferentialIsSelected = document.getElementById('MaltipleGameModeOnDifferential');


    if (checkingDifferentialIsSelected.checked == true) {
        selectedGameModeInMultipleMode['Differential'] = 1;
    } else {
        selectedGameModeInMultipleMode['Differential'] = 0;
...
// Checking Target Is Selected Or Not 
function selectTarget() {
    ...
    if (IschangePreferencesOn == 1) var checkingTargetIsSelected = document.getElementById('MaltipleGameModeOnTargetINCP');
    else var checkingTargetIsSelected = document.getElementById('MaltipleGameModeOnTarget');

    if (checkingTargetIsSelected.checked == true) {
        selectedGameModeInMultipleMode['Target'] = 1;
    } else {
        selectedGameModeInMultipleMode['Target'] = 0;
...
function selectAllMode() {
    ...
    if (checkingSOWIsSelected.checked == true) {
        ...
        selectedGameModeInMultipleMode['Differential'] = 1;
        document.getElementById('MaltipleGameModeOnDifferential').checked = true;
        selectedGameModeInMultipleMode['Target'] = 1;
        document.getElementById('MaltipleGameModeOnTarget').checked = true;
        selectedGameModeInMultipleMode['ZigZagPlayer'] = 1;
        ...
        selectedGameModeInMultipleMode['Differential'] = 0;
        document.getElementById('MaltipleGameModeOnDifferential').checked = false;
        selectedGameModeInMultipleMode['Target'] = 0;
        document.getElementById('MaltipleGameModeOnTarget').checked = false;
        selectedGameModeInMultipleMode['ZigZagPlayer'] = 0;
...
function balanceManagemrnt(winner) {
    ...
    if (winner == 'Player') {
        ...
        // brake mode is on 
        if (BoradForMultiMode.length >= 1 && BoradForMultiMode[BoradForMultiMode.length - 1].BrakeCount >= brakeValue) {
    ...
    if (winner == "Banker") {
        balanceMultipleMode += multipleBetUnitOnBanker * (PerUnit * comission4B / 100);
        balanceMultipleMode -= multipleBetUnitOnPlayer * PerUnit;
        netProfitMultiMode -= multipleBetUnitOnPlayer;
        netProfitMultiMode += multipleBetUnitOnBanker;
    ...
    balanceMultipleMode = roundOff(balanceMultipleMode);
...
function pushInTableINMultiMode() {
    ...
    tempSaveObject[5] = selectedGameModeInMultipleMode['Differential'];
    tempSaveObject[6] = selectedGameModeInMultipleMode['Target'];
    tempSaveObject[7] = selectedGameModeInMultipleMode['MVD'];
...
function changeDataTable() {
    ...
    for (i = 0; i < Borad.length; i++) {
        ...
        if (setChangeByNewAddMethodInGame == 0 && IschangePreferencesOn == 0) {
            listOfSelectedGameMode.push(BoradForMultiMode[i].selectedGameMode);
        }
    ...
    while (i < listOfWinner.length) {
        ...
        if (setChangeByNewAddMethodInGame == 0 && IschangePreferencesOn == 0) {
            ...
            selectedGameModeInMultipleMode['Differential'] = listOfSelectedGameMode[i][5];
            selectedGameModeInMultipleMode['Target'] = listOfSelectedGameMode[i][6];
            selectedGameModeInMultipleMode['MVD'] = listOfSelectedGameMode[i][7];
        }
        ...
        if (setChangeByNewAddMethodInGame == 0 && IschangePreferencesOn == 1 && IsTakeTieFormOldMOde == 1) {
            ...
            selectedGameModeInMultipleMode['Differential'] = storelistOfSelectedGameMode[i][5];
            selectedGameModeInMultipleMode['Target'] = storelistOfSelectedGameMode[i][6];
            selectedGameModeInMultipleMode['MVD'] = storelistOfSelectedGameMode[i][7];
        }
    ...
    listOfWinner = [];
    whenPlay2ndOn = [], whenPlay3rdOn = [], whenTieModeOn = [];
...
function ChangePlayMethodInMultiMode(methodType) {
    selectedMethodType = methodType.options[methodType.selectedIndex].innerHTML;
    changeDataTable();
...
function displayAmtInLable() {
    ...
    html = ` Differential `;
    if (Borad.length == 0) html += `<span>(0)</span>`;
    else if (0 > totalBalanceInMMInDiff) html += "<span class='boradTableinBanker'>(" + totalBalanceInMMInDiff + ")</span>";
    else html += "<span>(" + totalBalanceInMMInDiff + ")</span>";
    if (IschangePreferencesOn == 0) document.getElementById("differentialMMLable").innerHTML = html;
    if (IschangePreferencesOn == 1) document.getElementById("differentialMMLableINCP").innerHTML = html;

    html = ` Target `;
    if (Borad.length == 0) html += `<span>(0)</span>`;
    else if (0 > totalBalanceInMMInTarget) html += "<span class='boradTableinBanker'>(" + totalBalanceInMMInTarget + ")</span>";
    else html += "<span>(" + totalBalanceInMMInTarget + ")</span>";
    if (IschangePreferencesOn == 0) document.getElementById("targetMMLable").innerHTML = html;
    if (IschangePreferencesOn == 1) document.getElementById("targetMMLableINCP").innerHTML = html;
...
function calindividualBalance(actualWinner) {
    ...
    if (selectedGameModeInMultipleMode['Differential'] == 1) {
        totalBalanceInMMInDiff += indiviualDiffBalanceInMM;
        totalBalanceInMMInDiff = roundOff(totalBalanceInMMInDiff, 2);
    }

    if (selectedGameModeInMultipleMode['Target'] == 1) {
        totalBalanceInMMInTarget += indiviualTargetBalanceInMM;
        totalBalanceInMMInTarget = roundOff(totalBalanceInMMInTarget, 2);
    }
